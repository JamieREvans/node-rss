...
function generateXML (data){
    ...
    data.items.forEach(function(item) {
        ...
        if (item.custom_elements && data.itemKey != 'item') {
          item.custom_elements.forEach(function(item) {
            if (item) {
              for (var key in item) {
                item[key] = { _cdata: item[key] };
              }
            }
          });
        }
    ...
    console.log(data.feedKey);
    if (data.feedKey) {
      xml = {};
      xml[data.feedKey] = channel;
      return xml;
    }
...
function RSS (options, items, feedKey, itemKey) {
    options = options || {};
